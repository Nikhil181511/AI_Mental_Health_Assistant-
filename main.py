...
PERPLEXITY_API_KEY="pplx-1SH8YQaVRAi9Bx9jLx3B984KjzokcI5Z2XKgMtSLZ7EzgS"
...
def detect_mood_with_ollama(user_input: str):
    try:
        prompt = (
            ...
            "Analyze the user's input and reply with only one word describing their emotional mood. "
            "Possible moods: Sad, Happy, Angry, Anxious, Neutral. "
            f"Input: '{user_input}'\nMood:"
        )
        model = genai.GenerativeModel(model_name="models/gemini-2.0-flash")
        response = model.generate_content(prompt)
        mood = response.text.strip().split()[0].lower()
        mood_map = {
...
def recommend(mood: MoodRequest):
    ...
    if any(x in detected_mood for x in ["sad", "depressed"]):
        mood_key = next((x for x in coping_keywords if x in detected_mood), "sad")
        query = random.choice(coping_keywords[mood_key])
        videos = get_youtube_videos(f"self-care tips and uplifting activities for {detected_mood} mood", 5)
        articles = get_news_articles(query, 5)
        product = get_self_care_product("stress relief, fidget spinner")

    elif "anxious" in detected_mood:
        videos = get_youtube_videos("guided meditation and relaxation for anxiety relief", 5)
        articles = get_news_articles("calm anxious mind", 5)
        product = get_self_care_product("calming products, anxiety relief")

    elif any(x in detected_mood for x in ["happy", "excited"]):
        videos = get_youtube_videos("fun activities and positive habits for happiness", 5)
        ...
        videos = get_youtube_videos("anger management and calming techniques", 5)
        articles = get_news_articles("how to control anger", 5)
        product = get_self_care_product("stress relief balls, calming tea")

    else:
        videos = get_youtube_videos(f"self-care and wellness tips for {detected_mood} mood", 5)
